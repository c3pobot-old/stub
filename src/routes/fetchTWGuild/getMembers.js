...
const getCachePlayers = async(memberIds = [], collection = 'twPlayerCache', project)=>{
  try{
    if(project) project.playerId = 1
    return await mongo.find(collection, {_id: { $in: memberIds}}, project)
  }catch(e){
    console.error(e);
  }
}
const getNewPlayers = async(missingMembers = [])=>{
  try{
    ...
    while(i--) res.push(GetPlayer({playerId: missingMembers[i].playerId, allyCode: missingMembers[i].allyCode?.toString()}, { collection: 'twPlayerCache' }))
